...
export class ChatService {
	...
	public sendMessage = (user: string, message: string): Observable<void> => {
		return from(this.hubConnection.invoke("SendMessage", user, message)).pipe(
	...
	private subscribeToReceiveMessageEvent(handler: (value: Message) => void): void {
		this.hubConnection.on("ReceiveMessage", (sender, content) => handler({ sender, content }))
	}

	private unsubscribeFromReceiveMessageEvent(handler: (value: Message) => void): void {
