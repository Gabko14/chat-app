...
export class NotificationService {
  ...
  addPushSubscriber(fcmRegistrationToken: string) {
    return this.http.post(this.notificationsEndpoint + '/add-subscriber', null, {params: {fcmRegistrationToken}})
  ...
  sendToEveryone(notification: Notification) {
    return this.http.post(this.notificationsEndpoint + '/send-everyone-notification', notification)
      .pipe(catchError((error: any): Observable<any> => {
        console.error('There was an error!', error);
        return of(error);
      }));
